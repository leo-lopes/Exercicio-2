...
u[0]=0.0
u[1:] =np.exp(-(x[1:]-2)**2/0.1)
s[0,:]= v1*(5.)*(1/10.)*(x-2)*np.exp(-5*(x-2)**2)
s[0,0] = 0.0
s[0,int(L/dx)-1] = 0.0
r[0,:] = v1*(-5.)*(1/10.)*(x-2)*np.exp(-5*(x-2)**2)
r[1,:] = r[0,:]
v=np.zeros(int(L/dx))
...
for i in range (0,nsteps):
    ...
    for j in range (1,len(u)-1):
        ...
        s[2,j] = s[0,j] +((r[1,j+1]-r[1,j-1])*v[j]) - ((v[j+1]-v[j-1])*r[1,j])
    s[2,0] = 0.0
    s[2,int(L/dx)-1] =0.0
    r[2,0] = r[2,1]
    ...
    if (c%nplot == 0):
        ...
        plt.clf()
os.system("ffmpeg -r 5 -y -i 'foo%03d.jpg' ex2-2.m4v")
os.system("rm -f *.jpg")
